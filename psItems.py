...
class Array(Expr):
    ...
    #needs to be updated
    def evaluate(self,psstacks):
        psstacks.opstack.append("mark") # push mark to stack
        for item in self.value:
            item.evaluate(psstacks) # evaluate all items in Array
        newarr = []
        while True: # pop all items left on opstack up until the mark
            val = psstacks.opstack.pop() 
            if val != "mark":
                newarr.append(val) # and add them to array
            else:
                break
        newarr.reverse()
        psstacks.opstack.append(ArrayValue(newarr)) # append new evaluated ArrayValue onto opstack
...
class Name(Expr):
    ...
    def evaluate(self,psstacks):
        if self.var_name[0] == '/': # name is a variable, push directly onto stack
            psstacks.opstack.append(self.var_name)
        elif self.var_name in psstacks.builtin_operators: # name is an operator
            exe = psstacks.builtin_operators[self.var_name] # recieves the function from the dictionary
            exe() # executes the function
        else: # name is a function/variable
            if isinstance(psstacks.lookup(self.var_name), FunctionValue): # function
                psstacks.lookup(self.var_name).apply(psstacks) # function is applied
            else: # variable
                psstacks.opstack.append(psstacks.lookup(self.var_name)) # variable is pushed onto opstack
...
class Block(Expr):
    ...
    def evaluate(self, psstacks):
        psstacks.opstack.append(FunctionValue(self.value)) # push new FunctionValue onto stack
